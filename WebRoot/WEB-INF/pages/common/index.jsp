...
<script type="text/javascript">
	...
	$(function() {
		...
		// 系统管理菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/functionAction_findSysMenu',
			type : 'POST',
			dataType : 'text',
			success : function(data) {
				var zNodes = eval("(" + data + ")");
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("房屋销售系统 v1.0","管理员邮箱: ibesth520@gmail.com");
	}
</script>
</head>
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="sessionInfoDiv"
			...
			[<strong>${existUser.username}</strong>]，欢迎你！
		</div>
		...
		if(v){
			...
			if(v1==v2){
				$.post("${pageContext.request.contextPath }/userAction_editPassword",{"password":v1},function(data){
					if(data == "1"){
						$.messager.alert("提示消息","密码修改成功","info");
					}else {
						$.messager.alert("提示消息","密码修改失败","warning")
					}
					$("#editPwdWindow").window("close");
				});
			}
